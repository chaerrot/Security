...
<div class="container">
	...
	<c:choose>
		...
		<c:otherwise>
			<!-- 
				security-context2.xml에서 login-processing-url을 성정한
				경우 동일한 값으로 action속성을 설정해야 한다.
				설정하지 않았다면 default 값이 "/login"이므로 이와 같이 기술한다.
				
				만약 해당 설정값을 추가했다면 컨텍스트 루트 경로를 제외한 나머지 경로를
				통해 action속성을 기술해야 한다.
			 -->
			...
			<form:form name="loginFrm" action="../myLoginAction" method="post">
				<!-- 
					...
					이와 같이 기술하였으므로 로그인에 실패한 경우에는 아래와 같이 처리하면 된다.
				 -->
				...
				<!-- 
					...
					password-parameter="pass" 이와 같이 설정되어 있으므로 name 속성은
					아래와 같이 기술한다.
				 -->
				<p>
